...
IF EXISTS (
  select 1 from information_schema.columns where table_name = 'ResourceChangeData'
...
--
-- STORED PROCEDURE
--     FetchResourceChangeData
--
-- DESCRIPTION
--     Returns resource change data within range
--
-- PARAMETERS
--     @startId
...
--     @delayMilliseconds (Optional)
--         * Delay time to fetch current records from resource change table.
--
-- RETURN VALUE
--     Resource change data rows.
--
CREATE OR ALTER PROCEDURE dbo.FetchResourceChangeData
    @startId BIGINT, 
    @pageSize BIGINT, 
...
BEGIN
    ...
    DECLARE @endId BIGINT = @startId + @pageSize
...
--     @isCaptureResourceChangeDataEnabled
...
CREATE OR ALTER PROCEDURE dbo.UpsertResource_4
    ...
    @isCaptureResourceChangeDataEnabled bit = 0
AS
    SET NOCOUNT ON
	...
	IF (@isCaptureResourceChangeDataEnabled) = 1 BEGIN
        --If the capture resource changes feature is enabled, to execute a stored procedure called CaptureResourceChanges to insert resource change data.
		EXEC CaptureResourceChanges @isDeleted=@isDeleted, @version=@version, @resourceId=@resourceId, @resourceTypeId=@resourceTypeId
    END
