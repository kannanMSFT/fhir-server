...
namespace Microsoft.Health.Fhir.Core.Features.Resources.Delete
{
    public class ConditionalDeleteResourceHandler : BaseResourceHandler, IRequestHandler<ConditionalDeleteResourceRequest, DeleteResourceResponse>
    {
        ...
        public async Task<DeleteResourceResponse> Handle(ConditionalDeleteResourceRequest request, CancellationToken cancellationToken)
        {
            ...
            (IReadOnlyCollection<SearchResultEntry> matchedResults, string ct) = await _searchService.ConditionalSearchAsync(request.ResourceType, request.ConditionalParameters, cancellationToken);
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        foreach (IEnumerable<SearchResultEntry> batch in matchedResults.TakeBatch(10))
                        {
                            ...
                            if (itemsDeleted >= MaxDeletedItems)
                            {
                                break;
                            }
                        }
                        ...
                        if (!string.IsNullOrEmpty(ct))
                        {
                            (matchedResults, ct) = await _searchService.ConditionalSearchAsync(
                                ...
                                cancellationToken,
                                new[] { Tuple.Create(KnownQueryParameterNames.ContinuationToken,  Convert.ToBase64String(Encoding.UTF8.GetBytes(ct))), new Tuple<string, string>(KnownQueryParameterNames.Count, MaxDeletedItems.ToString(CultureInfo.InvariantCulture)) });
                        }
                        else
                        {
